...
const subtotalLabel = computed(() =>
    `Subtotal: ${formatPrice(cartStore.totals.subtotal)}`
)

const shippingLabel = computed(() =>
    cartStore.totals.shipping > 0
        ? `Shipping: ${formatPrice(cartStore.totals.shipping)}`
        : 'Shipping: Not calculated'
)

const taxLabel = computed(() =>
    `Tax at 20%: ${formatPrice(cartStore.totals.tax)}`
)

const totalLabel = computed(() =>
    `Total: ${formatPrice(cartStore.totals.total)}`
)
...
<template>
  <aside
      class="bg-magnolia rounded-lg shadow p-6"
      role="region"
  ...
  >
    ...
    <dl class="space-y-4">
      ...
      <div class="flex justify-between items-center">
        <dt class="text-base text-navy-blue">
          Subtotals:
        </dt>
        <dd
            class="text-base font-semibold text-navy-blue"
      ...
      <div class="flex justify-between items-center">
        ...
        >
          ...
          <span v-else class="text-navy-blue">
            —
          </span>
        </dd>
      </div>
      ...
      >
        <!-- Total -->
        <div class="flex justify-between items-center">
          <dt class="text-lg font-bold text-navy-blue">
            Totals:
          </dt>
          <dd
              class="text-lg font-bold text-navy-blue"
