...
<template>
  <section aria-labelledby="shipping-calculator-heading">
    <h2 id="shipping-calculator-heading" class="text-md font-bold text-navy-blue mb-6">
      Calculate Shopping
    </h2>

    <div v-if="hasErrors" role="alert" aria-live="assertive" class="sr-only">
      Form has errors. Please review the fields.
    </div>

    <form class="bg-magnolia rounded-lg shadow p-6 space-y-6" novalidate @submit.prevent="handleCalculateShipping">
      <div>
      ...
      <div>
        <label for="shipping-state" class="sr-only">State or Region</label>
        <input
          id="shipping-state"
      ...
      <div>
        <label for="shipping-zipcode" class="sr-only">Zip Code</label>
        <input
          id="shipping-zipcode"
      ...
      <UiButton type="submit" block size="lg" :loading="isCalculating" :disabled="isCalculating">
